...
실행 환경: mineru가 설치된 파이썬(= s1과 같은 demo_venv)으로 실행해야 한다
  (torch/torchvision/transformers 전부 이미 그 안에 있어 --with-classifier 도 별도 venv 불필요).
  예) <demo_venv>/Scripts/python.exe pipeline/s2_onestop_mineru.py --doc industry_15 --with-chart-analysis
"""
