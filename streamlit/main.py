...
def has_real_image_cards(pdf_id: str) -> bool:
    ...
    대체한다(그 함수 자체 문서화된 동작 — 이 파일은 그 로직을 건드리지 않는다). 새로 업로드된
    PDF는 매번 새 uuid 기반 pdf_id를 쓰므로 이 폴더가 미리 존재할 수 없어, 실제로는 항상
    예시 카드로 대체된다 — 그 사실을 화면에 정직하게 알리기 위한 체크."""
...
def render_upload():
    ...
    if saved:
        ...
        if saved["used_fallback_images"]:
            st.warning(
                "이 문서는 MinerU로 사전 처리된 실제 이미지/차트 카드가 없어, 이미지 근거는 "
                "예시 데이터로 대체됐습니다 — 아래 답변/근거 중 [image] 출처는 실제 업로드 "
                "문서 내용이 아닐 수 있습니다."
            )
