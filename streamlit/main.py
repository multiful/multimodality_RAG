...
분류 -> 텍스트/표/이미지 브랜치 -> 임베딩 -> document_evidence 즉시 적재)을 그 자리에서 실행한다.
이미지/차트 브랜치는 MinerU CLI 원스톱 파서를 애드훅 업로드용으로 직접 호출하며(캡션·OCR 텍스트
근거, 차트 서술형 해석은 지연 문제로 기본 off), 실패해도 텍스트/표 근거만으로 계속 진행한다.
PDF와 질문만 입력해도 되며(종목 연결은 선택), 종목을 지정하지 않으면 이 PDF 자체를 가리키는
임시 식별자를 ticker로 써서 즉시 적재 -> 질의응답까지 그 자리에서 수행한다.
...
def render_upload():
    ...
    question = st.text_input(
        "질문", placeholder="예: 이 리포트에서 실적 전망은 어떻게 나와있어?", key="upload_question"
    )

    with st.expander("종목 연결 (선택)"):
        universe = load_ticker_universe()
        ticker_options = ["-- 지정 안 함 --"] + [f"{u['name']} ({u['ticker']})" for u in universe] + ["직접 입력"]
    ...
    if st.button("업로드 및 분석 시작", type="primary", disabled=disabled):
        ...
        linked = bool(ticker)
        effective_ticker = ticker or pdf_id
        tmp_path = Path(tempfile.gettempdir()) / f"{pdf_id}.pdf"
        tmp_path.write_bytes(uploaded.getvalue())

        try:
            with st.status("PDF 분석 중...", expanded=True) as status:
                counts = ingest_pdf(tmp_path, pdf_id, effective_ticker, status)
                has_document_evidence.clear()  # 방금 적재한 근거를 바로 조회할 수 있도록 캐시 무효화

                answer = None
                if question:
                    status.write("질문에 대한 답변 생성 중...")
                    answer = answer_question(effective_ticker, question)
    ...
    if result:
        ...
        if result.get("answer"):
            st.divider()
            st.subheader(f"'{result['question']}'에 대한 답변")
            st.markdown(result["answer"]["answer"])
            with st.expander("근거 보기"):
                st.text(result["answer"]["context"])

        if result["linked"] and st.button("이 종목 질문하러 가기 →"):
            st.session_state["page"] = "detail"
