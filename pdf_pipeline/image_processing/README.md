...
> **실행**: `python pdf_pipeline/image_processing/s2_onestop_mineru.py --doc {doc_id}`
> (의존 파일은 같은 폴더의 `common.py`, `figure_classifier.py`, `s1_parse.py`뿐 — 독립 실행 가능)

## 0. 이 모듈이 다루는 범위 (PRD와의 관계)

`docs/PRD_pdf_pipeline.md`의 라우팅 설계는 "이미지 → VLM으로 상장기업·브랜드 식별"을 그리고
있는데, 이 모듈은 그것과는 다른 하위 문제를 다룬다: **증권사 리서치 PDF 안의 차트·이미지 블록을
분류하고, 차트는 데이터(수치·추세)까지 해석해서 RAG 검색용 텍스트로 만드는 것**이다(로고/브랜드
식별이 아니라 차트 내용 이해가 목적). 브랜드·로고 식별이 필요한 이미지 라우팅 단계와는 별도
트랙으로 봐야 하며, 두 모듈을 어떻게 합칠지(혹은 각자 다른 인풋을 받을지)는 팀 논의가 필요하다.

## 1. 배경·범위

- 전체 시스템: 네이버 금융 리서치 PDF → MinerU 파싱 → 파트별 처리(이미지/테이블/텍스트) →
  Supabase 색인 → RAG 질의응답. 이 문서는 **이미지 파트**(`pdf_pipeline/image_processing/`)만 다룬다.
- 처리 대상: MinerU 블록 타입 `chart`와 `image`만. `table`은 판정하지 않고 인계 표시만 한다
  (텍스트/테이블 파트 소관 — 남의 테이블에 직접 쓰지 않음. 인계 파일 연동은 §7 참고).
...
```
python pdf_pipeline/image_processing/s2_onestop_mineru.py --doc {doc_id} [옵션]
