"""[21] 다른 섹터 PDF(K-Wave 산업 리포트, 73페이지, 표 104개)로 표 처리 파이프라인 일반화 검증.
baseline(pdfplumber만) vs 채택 파이프라인(Adaptive Router: SIMPLE->pdfplumber, COMPLEX->TATR)
비교 + Canonical Field Metadata 추출(엔터/음식료 섹터 필드가 실제로 매칭되는지) 검증.
"""
...
RESULT_PATH = OUT_DIR / "result_kwave_table_validation.json"
...
def main():
    print(f"대상 PDF: {KWAVE_PDF.name}", flush=True)
    ...
    result = {
        "pdf": KWAVE_PDF.name, "n_tables": len(routed), "n_simple": n_simple, "n_complex": n_complex,
