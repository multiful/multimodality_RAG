...
RESULT_PATH = PDF_DIR / "result_pipeline_v4_entities.json"
REPORT_PATH = ROOT / "pdf_pipeline" / "table_processing" / "실험_v4_anchor_recall_report.md"
MAX_WORKERS = 8
...
def main():
    ...
    page_level_timing = []
    table_level_timing = []
    page_entities_by_page = {}

    # A) 페이지 레벨: 본문 + 이미지설명만 (표 제외 -> 항상 짧음) — 전 페이지 먼저 수행
    for p in memory["pages"]:
        ...
        page_entities_by_page[p["page"]] = page_entities

    # 문서 앵커 자동 산출: 페이지레벨 결과 전체에서 가장 자주 등장하는 후보(최빈값) —
    # 하드코딩 없이 매 문서마다 자동으로 다르게 뽑힘(일반화 목적)
    candidate_counts = {}
    for ents in page_entities_by_page.values():
        for part in re.split(r"[\n,]", ents):
            part = re.sub(r"\([^)]*\)", "", part).strip(" -·")
            if part:
                key = part.lower().replace(" ", "")
                candidate_counts.setdefault(key, {"count": 0, "display": part})
                candidate_counts[key]["count"] += 1
    doc_anchor = max(candidate_counts.values(), key=lambda v: v["count"])["display"] if candidate_counts else ""
    print(f"\n[anchor] 자동 산출된 문서 앵커: '{doc_anchor}' (페이지레벨 결과 최빈값)\n", flush=True)

    # B) 표 레벨: 표 1개씩 개별 호출, 문서 앵커를 짧게 prepend(맥락 상실로 인한 할루시네이션 완화)
    for p in memory["pages"]:
        table_entities_list = []
        for t_idx, md in enumerate(docling_tables_by_page.get(p["page"], []), start=1):
            anchored_context = f"[이 표는 '{doc_anchor}' 관련 문서에 포함된 표입니다]\n[표]\n{md}" if doc_anchor else f"[표]\n{md}"
            t0 = time.perf_counter()
            te = text_generate(model, processor, device, ENTITY_PROMPT_TMPL.format(context=anchored_context),
                                max_new_tokens=100)
            elapsed = round(time.perf_counter() - t0, 3)
        ...
        per_page_entities[p["page"]] = "\n".join([page_entities_by_page[p["page"]]] + table_entities_list)
    ...
    lines = [
        ...
        "",
        f"자동 산출된 문서 앵커: **'{doc_anchor}'**",
        "",
        "## 성능 지표",
        "",
        "| 지표 | v2+penalty(통짜) | v3(표 분리, 앵커 없음) | **v4(표 분리+앵커)** |",
        "|---|---|---|---|",
        f"| Recall | 90.0% (9/10) | 70.0% (7/10) | **{recall:.1%} ({len(hits)}/{len(target_set)})** |",
        f"| Precision(근사) | 83.3% | 95.9% | **{precision:.1%}** |",
        f"| F1(근사) | 86.5% | 80.9% | **{f1:.1%}** |",
        f"| 총 처리시간 | 1097.5초 | 528.1초 | **{total_pipeline_s}초** |",
        "",
        "### 구간별 지연",
