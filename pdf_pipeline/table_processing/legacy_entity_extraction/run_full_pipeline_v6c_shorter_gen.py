"""[14] 자가 피드백 Round 2: v6에서 page2가 근거 없는 회사명을 연쇄 생성(할루시네이션)한 원인 중 하나로
"자연스러운 멈춤 지점 없이 max_new_tokens=200까지 채우려는 경향"을 의심 — max_new_tokens을 150으로
줄여 생성 여유를 줄이면 할루시네이션이 원천적으로 줄어드는지(그러면서도 page3처럼 정상적으로 긴
엔티티 목록은 잘리지 않는지) 검증. 나머지 로직은 v6과 동일(표 타입 분기 등).
"""
...
RESULT_PATH = OUT_DIR / "result_pipeline_v6c_shorter_gen_entities.json"
REPORT_PATH = ROOT / "pdf_pipeline" / "table_processing" / "실험_v6c_shorter_gen_recall_report.md"
MAX_WORKERS = 8
...
def run_table_stage():
    ...
    crop_dir = OUT_DIR / "table_crops_router_complex_v6c"
