...
OUT_DIR = PDF_DIR
RESULT_PATH = OUT_DIR / "result_pipeline_v2_reppenalty_entities.json"
REPORT_PATH = ROOT / "pdf_pipeline" / "table_processing" / "실험_v2_reppenalty_recall_report.md"
MAX_WORKERS = 8
...
def main():
    ...
    lines = [
        ...
        "|---|---|---|---|",
        f"| Recall | 60.0% (6/10) | 80.0% (8/10) | **{recall:.1%} ({len(hits)}/{len(target_set)})** |",
        f"| Precision(근사) | 83.3% | 83.3% | **{precision:.1%}** |",
        f"| F1(근사) | 69.8% | 81.6% | **{f1:.1%}** |",
        f"| 총 처리시간 | 1105.1초 | 942.1초 | **{total_pipeline_s}초** |",
        "",
        "### 구간별 지연",
