...
OUT_DIR = PDF_DIR
RESULT_PATH = OUT_DIR / "result_pipeline_v2_entities.json"
REPORT_PATH = ROOT / "pdf_pipeline" / "table_processing" / "실험_v2_recall_report.md"
MAX_WORKERS = 8
...
def main():
    ...
    lines = [
        ...
        f"| Precision(lenient 근사) | 70.0% (7/10) | **{precision:.1%}** |",
        f"| F1(근사) | 64.6% | **{f1:.1%}** |",
        f"| 총 처리시간 | 468.7초 | **{total_pipeline_s}초** |",
        "",
        "### 구간별 지연 (v2)",
        f"- 페이지분류+텍스트+이미지설명(Qwen2.5-VL): {classify_text_image_s:.1f}초 (변경 없음, 기존 재사용)",
        f"- 표 구조화(YOLOv11 Crop + Docling 동적병렬): {docling_elapsed}초",
        f"- 엔티티 추출(Qwen2.5-VL): {entity_extract_total_s}초",
        "",
