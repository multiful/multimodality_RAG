"""[21] K-Wave 산업 리포트(73페이지) 엔티티 추출 검증 — 텍스트+표(비재무) 기반, VLM 이미지 캡셔닝은
시간상 제외(73페이지 전체 VLM 캡셔닝은 ~80분 추정이라 이번 검증 범위 밖으로 명시). 타겟 기업명은
각 기업 섹션 제목에 텍스트로 그대로 노출되므로(예: "하이브 [352820]") 이 스코프로도 recall 검증에
의미가 있음.
"""
...
KWAVE_PDF = ROOT / "pdf_pipeline" / "reference" / "KWave" / "20260721_industry_65157000.pdf"
MODEL_PATH = ROOT / "models" / "Qwen2.5-VL-7B-Instruct"
OUT_DIR = Path(__file__).resolve().parent
RESULT_PATH = OUT_DIR / "result_kwave_entity_extraction.json"

TATR_DPI = 300
TATR_TOP_PAD_PT = 35 / (150 / 72)
TATR_SIDE_PAD_PT = 12 / (150 / 72)

adaptive_table_router.PDF_PATH = KWAVE_PDF
...
def build_page_content():
    ...
    model.eval()
    doc_fitz = fitz.open(str(KWAVE_PDF))
    pdf_pp = pdfplumber.open(str(KWAVE_PDF))
    ...
    pdf_pp.close()
    doc_fitz.close()
    return page_text, tables_by_page, n_pages, n_finance_skipped
...
def main():
    ...
    page_text, tables_by_page, n_pages, n_finance_skipped = build_page_content()
    table_stage_s = round(time.perf_counter() - t0, 2)
    print(f"[table] 표 단계 총 소요: {table_stage_s}s (재무제표 스킵 {n_finance_skipped}개)", flush=True)
    ...
    result = {
        "pdf": KWAVE_PDF.name, "n_pages": n_pages, "n_finance_skipped": n_finance_skipped,
