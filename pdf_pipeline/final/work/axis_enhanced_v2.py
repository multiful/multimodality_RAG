...
def main():
    ...
    all_chunks = text_chunks + table_chunks + [caption_chunk]
    full_text = "\n".join(c["text"] for c in all_chunks)
    capcount = C.count_captions(full_text)

    out = {
        "axis":"enhanced_v2",
        "parse_time_s": round(classify_s+text_s+table_s+cap_s,3),
        "stage_timing":{"model_load_warmup_s":round(load_s,3),"page_classify_s":round(classify_s,3),
                         "text_route_s":round(text_s,3),"table_pdfplumber_s":round(table_s,3),"caption_recover_s":round(cap_s,3)},
        "total_time_s": round(load_s+classify_s+text_s+table_s+cap_s,3),
        ...
        "page_pred": page_pred,
        "routing":{"route_to_mineru":tp.get("route_to_mineru"),"hard_page_numbers":tp.get("hard_page_numbers"),
                    "n_hard_pages":len(tp.get("hard_page_numbers") or [])},
    }
    C.dump_json(OUT,out)
    print(f"[enhanced_v2] load {load_s:.1f}s classify {classify_s:.1f}s text {text_s:.1f}s "
          f"table(pdfplumber) {table_s:.1f}s caption {cap_s:.1f}s -> parse {out['parse_time_s']}s")
    print(f"  chunks={len(all_chunks)} (text {len(text_chunks)} + table {len(table_chunks)}) tables={n_tables} rows={n_rows}")
    print(f"  charts={len(capcount['chart_titles'])}/93 tabcaps={len(capcount['table_caps'])}/11 hard={tp.get('hard_page_numbers')}")
