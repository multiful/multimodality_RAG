...
OUT = HERE/"out_enhanced_v5.json"
DOC_TITLE = "스마트폰 수요 우려는 예견된 수순 (반도체 및 소부장 Weekly)"
CAP_RE = re.compile(r"^\s*(도표|표)\s*\d+\.?\s*.*$")
_NUM = re.compile(r"^[\d\.,%\-\(\)\s]+$")
def _ts_gate(tbl):
    """text-strategy 출력이 '깨끗한 정형/수치표'인가 판정 → True=text-strategy, False=word-clustering."""
    ...
    cons = sum(1 for c in counts if c == modal) / len(counts)
...
def main():
    ...
    # (1) 표 재선형화 v5 — 하이브리드 게이트(FINAL):
    #     text-strategy 출력이 '깨끗한 정형/수치표'면 text-strategy(컬럼 완전),
    #     불규칙 텍스트표면 word-clustering(회사명 분할 방지). 15문서 검증에서 v4 회귀 해소.
    ...
    for p in cls["pages"]:
        pg=p["page"]
        tboxes=[rect for (cn,rect) in page_boxes[pg] if cn=="Table"]
        ...
        for ti,rect in enumerate(tboxes,1):
            ...
            if _ts_gate(tbl):  # 깨끗한 정형/수치표 -> text-strategy
                for row in tbl:
                    cells=[("" if c is None else str(c).replace("\n"," ").strip()) for c in row]
                    if any(cells): rows_md.append("| "+" | ".join(cells)+" |")
            else:  # 불규칙표 -> word-clustering(이름 보존)
                try:
                ...
                if not rows_md and tbl:
                    for row in tbl:
                        cells=[("" if c is None else str(c).strip()) for c in row]
                        if any(cells): rows_md.append("| "+" | ".join(cells)+" |")
            if not rows_md: continue
            n_tables+=1; n_rows+=len(rows_md)
            table_chunks.append({"text": f"[표 p{pg} #{ti}]\n"+"\n".join(rows_md),
                                  "kind":"table", "page":pg, "table_idx":ti})
    table_s=time.time()-t
    ...
    C.dump_json(OUT,out)
    print(f"[enhanced_v5] load {load_s:.1f}s classify {classify_s:.1f}s text {text_s:.1f}s "
