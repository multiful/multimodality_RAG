...
def main():
    ...
    # (1) 표 재선형화 v4 — pdfplumber text-strategy(텍스트 정렬 기반)로 교체.
    #     TATR-grid가 넓은 표 우측열을 truncation하던 문제 근본해결(구조화 레코드 완전).
    #     TATR 추론 제거 → 더 빠름. 실패 시 word-clustering 폴백.
    ...
    for p in cls["pages"]:
        ...
        for ti,rect in enumerate(tboxes,1):
            ...
            rows_md=[]
            try:
                crop=page_pp.within_bbox((x0,y0,x1,y1))
                tbl=crop.extract_table(TS)
                maxcol=max((len(r) for r in (tbl or [])), default=0)
                if tbl and maxcol>=2:
                    for row in tbl:
                ...
                rows_md=[]
            if not rows_md:  # 폴백: word-clustering
                try:
                    rows=parse_simple_table_from_words(page_pp,(x0,y0,x1,y1),12.0)
                    for row in rows:
    ...
    C.dump_json(OUT,out)
    print(f"[enhanced_v4] load {load_s:.1f}s classify {classify_s:.1f}s text {text_s:.1f}s "
